...
exports.addCourse = asyncHandler(async (req, res, next) => {
  ...
  if (bootcamp.user.toString() !== req.user.id && req.user.role !== "admin") {
    return next(
      new ErrorResponse(
        `User #${req.user.id} Not Allowed to ADD this course to the bootcamp #${bootcamp._id}`,
        401
      )
    );
  }
...
exports.updateCourse = asyncHandler(async (req, res, next) => {
  ...
  if (course.user.toString() !== req.user.id && req.user.role !== "admin") {
    return next(
      new ErrorResponse(
        `User #${req.user.id} Not Allowed to UPDATE the course #${course._id}`,
        401
      )
    );
  }
...
exports.deleteCourse = asyncHandler(async (req, res, next) => {
  ...
  if (course.user.toString() !== req.user.id && req.user.role !== "admin") {
    return next(
      new ErrorResponse(
        `User #${req.user.id} Not Allowed to DELETE the course #${course._id}`,
        401
      )
    );
  }
